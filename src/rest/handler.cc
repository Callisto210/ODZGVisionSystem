...
void streaming_handler::operator()() {
    ...
    conf->video.fps = -1;
    conf->audio.audio_bitrate = -1;
    conf->video.video_bitrate = -1;
    conf->video.width = -1;
    conf->video.height = -1;
    conf->port = -1;
	...
	if(doc.HasMember("uri")) {
		...
		if(doc["fps"].IsInt()) {
		    conf->video.fps = doc["fps"].GetInt();
		}
		else {
		    conf->video.fps = std::atoi(doc["fps"].GetString());
		}
	}
	...
	if(doc.HasMember("acodec")) {
		if(doc["acodec"].IsString())
        		conf->audio.acodec = doc["acodec"].GetString();
	}

	if(doc.HasMember("vcodec")) {
		if(doc["vcodec"].IsString())
        		conf->video.vcodec = doc["vcodec"].GetString();
	}

	if(doc.HasMember("audio_stream")) {
		if(doc["audio_stream"].IsString())
        		conf->audio.audio_stream = doc["audio_stream"].GetString();
	}

	if(doc.HasMember("video_stream")) {
		if(doc["video_stream"].IsString())
        		conf->video.video_stream = doc["video_stream"].GetString();
	}
	...
	if(doc.HasMember("video_bitrate")) {
		if(doc["video_bitrate"].IsInt())
			conf->video.video_bitrate = doc["video_bitrate"].GetInt();
		else
			conf->video.video_bitrate = std::atoi(doc["video_bitrate"].GetString());
	}

	if(doc.HasMember("audio_bitrate")) {
		if(doc["audio_bitrate"].IsInt())
			conf->audio.audio_bitrate = doc["audio_bitrate"].GetInt();
		else
			conf->audio.audio_bitrate = std::atoi(doc["audio_bitrate"].GetString());
	}

	if(doc.HasMember("width")) {
		if(doc["width"].IsInt())
			conf->video.width = doc["width"].GetInt();
		else
			conf->video.width = std::atoi(doc["width"].GetString());
	}

	if(doc.HasMember("height")) {
		if(doc["height"].IsInt())
			conf->video.height = doc["height"].GetInt();
		else
			conf->video.height = std::atoi(doc["height"].GetString());
	}
